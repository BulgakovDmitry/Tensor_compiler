...
Codegen::Codegen() {
    registry_.insert<mlir::func::FuncDialect, mlir::arith::ArithDialect, mlir::tensor::TensorDialect>();
...
mlir::OwningOpRef<mlir::ModuleOp> Codegen::generate(const Graph &graph) {
    ...
    std::string func_name = graph.get_name().empty() ? "main" : graph.get_name();
    ...
    for (const auto &input_name : graph.get_inputs()) {
        ...
        if (!tensor) {
            throw std::runtime_error("graph input tensor not found: " + input_name);
        }
    ...
    for (const auto &output_name : graph.get_outputs()) {
        ...
        if (!tensor) {
            throw std::runtime_error("graph output tensor not found: " + output_name);
        }
        result_types.push_back(convert_tensor_type(*tensor));
    }

    mlir::FunctionType func_type = builder.getFunctionType(input_types, result_types);
    mlir::func::FuncOp func = mlir::func::FuncOp::create(loc, func_name, func_type);
    ...
    if (graph.get_outputs().empty()) {
        ...
        if (entry_block->getNumArguments() == 0) {
            throw std::runtime_error("cannot return function argument: graph has outputs but no inputs");
        }
...
mlir::RankedTensorType Codegen::convert_tensor_type(const Tensor &tensor) const {
