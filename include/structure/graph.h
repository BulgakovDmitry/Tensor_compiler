...
private:
  ...
  Tensor handleTensor(const onnx::TensorProto &t);
  ...
  Tensor handleTensor(const onnx::ValueInfoProto &t, const Tensor_kind &type);
  ...
  Node handleNode(std::size_t &node_idx, const onnx::NodeProto &node);
};
