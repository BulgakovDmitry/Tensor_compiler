...
inline int extractElemType(const onnx::ValueInfoProto &v) {
  if (!v.has_type() || !v.type().has_tensor_type())
    return onnx::TensorProto_DataType_UNDEFINED;
  return v.type().tensor_type().elem_type();
}

inline std::vector<int64_t> extractDims(const onnx::ValueInfoProto &v) {
...
inline std::string extractTensorBytes(const onnx::TensorProto &t) {
  if (!t.raw_data().empty())
    return t.raw_data();
